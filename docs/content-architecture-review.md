...
| The publisher entity contradicts itself | `public/llms.txt:5` says "Predexyo UK Ltd"; `lib/site.ts:8` says `entity: 'Doefin'`, registered Port Louis, Mauritius. `sameAs` is empty. |
...
- `public/llms.txt:5` says "Published by **Predexyo UK Ltd**". `lib/site.ts` says `entity: 'Doefin'`, `registeredIn: 'Port Louis, Mauritius'`, and `app/layout.tsx:43` emits that as `legalName`. `grep -rn Predexyo` across `app content lib components public` returns that one llms.txt line. **The site tells search engines one legal publisher in JSON-LD and a different one in the file written for AI crawlers.** That is a stronger negative E-E-A-T signal than any missing bio.
